...
def criar_arquivo_pdf(titulo_documento: str, instrucao_ia: str) -> str:
    # ... (código da função criar_arquivo_pdf) ...
    try:
        prompt = f"Crie o conteúdo para um documento PDF com base na seguinte instrução: '{instrucao_ia}'. Responda APENAS com uma estrutura JSON..."
        ...
        file_path = f"{titulo_documento.replace(' ', '_')}.pdf"; pdf.output(file_path)
        return file_path
    except Exception as e: return f"Erro: {e}"

def criar_arquivo_planilha(nome_arquivo: str, instrucao_ia: str) -> str:
    # ... (código da função criar_arquivo_planilha) ...
    try:
        prompt = f"Crie os dados para uma planilha com base na seguinte instrução: '{instrucao_ia}'. Responda APENAS com uma estrutura JSON..."
        response = model_chat.generate_content(prompt)
        json_text = extrair_json(response.text)
...
# NOVA FERRAMENTA: O RELÓGIO DO ALFRED
def obter_data_e_hora_atual() -> dict:
    ...
    # Usando o fuso horário de São Paulo como referência
    tz = pytz.timezone('America/Sao_Paulo')
    now = datetime.now(tz)
    # Formata a data de uma maneira que a IA entenda bem
...
try:
    ...
    model_chat = genai.GenerativeModel(model_name="models/gemini-pro-latest", system_instruction=("Seu nome é Alfred..."))
    
    # Adicionamos a nova ferramenta ao cérebro de conversação
    model_tools = genai.GenerativeModel(
        model_name="models/gemini-pro-latest",
        tools=[tavily_search, obter_data_e_hora_atual], # <-- NOVA FERRAMENTA ADICIONADA AQUI
        system_instruction=("Seu nome é Alfred...")
    )
    
    logger.info("Modelos de IA (chat e ferramentas com data/hora) inicializados com sucesso.")
except Exception as e:
    logger.critical(f"FALHA CRÍTICA NA INICIALIZAÇÃO DE UM DOS MODELOS: {e}")

# --- 5. LÓGICA DOS COMANDOS DO BOT ---
# (As funções start, handle_chat, pdf_command, planilha_command permanecem as mesmas da versão 3.2)
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.chat_data.clear(); await update.message.reply_text("Olá, Senhor. O que deseja? Para gerar arquivos, use os comandos /pdf ou /planilha.")
...
async def handle_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    except Exception as e: await update.message.reply_text(f"Contratempo na conversa. Detalhe: {e}")

async def pdf_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # ... (código do pdf_command) ...
    ...
    await update.message.reply_text("Entendido. Preparando o documento PDF..."); await update.message.chat.send_action(action='upload_document')
    file_path = criar_arquivo_pdf("documento", instrucao)
    if "Erro:" not in file_path: await context.bot.send_document(chat_id=update.effective_chat.id, document=open(file_path, 'rb')); os.remove(file_path)
...
async def planilha_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # ... (código do planilha_command) ...
    if not model_chat: await update.message.reply_text("Perdão, Senhor. Meu módulo de documentação está com falha."); return
    instrucao = " ".join(context.args);
    if not instrucao: await update.message.reply_text("Uso: /planilha <instrução>"); return
    await update.message.reply_text("Entendido. Compilando os dados para a planilha..."); await update.message.chat.send_action(action='upload_document')
...
# --- 6. PONTO DE ENTRADA ---
def main() -> None:
    # Adicionar a biblioteca pytz para fusos horários
    # Esta linha é apenas para garantir que o Railway a instale.
    # O import no topo do arquivo já faz o trabalho.
    ...
    logger.info("Iniciando bot Alfred - Versão 3.3 (com Relógio).")
